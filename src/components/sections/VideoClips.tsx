...
const VideoClips = () => {
  ...
  return (
    ...
    >
      ...
      <Header useMotion={true} p="精彩内容" h2="片段欣赏" />
              ...
              <div className="bg-gradient-to-r from-purple-900/30 to-blue-900/30 rounded-lg p-4 border border-purple-500/20">
                <p className="text-white text-[16px]">
                  ...
                  <span className="text-[#e3d7b1] font-semibold">
                    {videoSeries.reduce((total, series) => total + series.videos.length, 0)}
                  </span>{' '}
                  个视频， 分为{' '}
                  <span className="text-[#e3d7b1] font-semibold">{videoSeries.length}</span> 个剧目
                </p>
              </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {series.videos.map((video, videoIndex) => (
                        ...
                        >
                          {/* 视频缩略图区域 */}
                          <div
                            className="aspect-video bg-gradient-to-br from-purple-900/30 to-blue-900/30 rounded-lg mb-3 relative overflow-hidden group cursor-pointer"
